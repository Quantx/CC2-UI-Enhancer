...
Variometer = {
    ...
    fuel = VarTimedHistory:new{ttl=30 * 60, interval=30, data={}},
    ...
    _render = function(self, pos, col)
        ...
        if fuel_samples > 0 then
            local fuel_per_min = self:fuel_burnrate() * 60.0
            ...
            update_ui_text(
                    ...
                    fuel_use_per_min,
                    64, 0, col, 0)
            if fuel_samples > 3 then
                -- calc the burn rate from 2 sec ago
                local early = update_get_logic_tick() - (5 * self.fuel.ticks_per_sec)
                local later = update_get_logic_tick() - (1 * self.fuel.ticks_per_sec)
                ...
                local one_sec_used = (delta / 4.0) * self.vehicle_full_fuel
